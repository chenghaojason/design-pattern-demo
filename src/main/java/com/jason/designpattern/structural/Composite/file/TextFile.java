...
public class TextFile extends AbstractFile {
    ...
    public void add(AbstractFile file) {
        try {
            throw new CompositeOperationException("文本不支持添加");
    ...
    public void remove(AbstractFile file) {
        try {
            throw new CompositeOperationException("文本不支持移除");
        } catch (CompositeOperationException e) {
            System.out.println(e);
        }
    }
    ...
    @Override
    public void display() {
        System.out.println("直接打开记事本：" + fileName + "（文件类型：" + fileType + ",文件大小：" + fileSize + "kb）");
    }
}
